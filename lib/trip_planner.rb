...
class TripPlanner
  ...
  def get_fixed_itineraries(from, to, trip_datetime, arriveBy, mode="TRANSIT,WALK", wheelchair="false", walk_speed=3.0, max_walk_distance=2, max_bicycle_distance=5, optimize='QUICK', num_itineraries = 3, min_transfer_time=nil, max_transfer_time=nil, try_count=Oneclick::Application.config.OTP_retry_count)
    try = 1
    result = nil
    response = nil

    while try <= try_count
      result, response = get_fixed_itineraries_once(from, to, trip_datetime, arriveBy, mode, wheelchair, walk_speed, max_walk_distance, max_bicycle_distance, optimize, num_itineraries, min_transfer_time, max_transfer_time)
      if result
        break
      else
  ...
  def get_fixed_itineraries_once(from, to, trip_datetime, arriveBy, mode="TRANSIT,WALK", wheelchair="false", walk_speed=3.0, max_walk_distance=2, max_bicycle_distance=5, optimize='QUICK', num_itineraries=3, min_transfer_time=nil, max_transfer_time=nil)
